...
char* format_file_size(long bytes) {
    ...
    int unit = 0; double s = (double)bytes;
...
static int is_image_file(const char *name) {
    const char *ext = strrchr(name, '.'); if (!ext) return 0; ext++;
...
// Locate a usable font file at runtime
...
// Render a single line of text as a texture; returns NULL on failure
...
// Draw text at (x,y); frees the texture afterwards
...
void update_window_title(App *app) {
    ...
    if (app->file_list.count > 0) {
        ...
        const char *f = strrchr(p, PATH_SEP); f = f ? f + 1 : p;
        char t[512];
...
void free_file_list(FileList *list) {
    ...
    free(list->paths); list->paths = NULL; list->count = 0; list->current = 0;
}
...
int scan_folder(const char *filepath, FileList *list) {
...
#endif
    if (sep) *sep = '\0'; else secure_strncpy(dir, ".", sizeof(dir));

    list->paths = malloc(MAX_IMAGES * sizeof(char *));
    if (!list->paths) return 0;
    list->count = 0; list->current = 0;

#ifdef _WIN32
    char search[MAX_PATH_LENGTH];
    snprintf(search, sizeof(search), "%s\\*", dir);
    WIN32_FIND_DATA ffd; HANDLE h = FindFirstFile(search, &ffd);
    ...
    struct dirent *e;
    while ((e = readdir(d)) && list->count < MAX_IMAGES)
...
SDL_Texture* get_thumb(App *app, int index) {
    ...
    SDL_Surface *full = IMG_Load(path); if (!full) return NULL;
    ...
    if (scaled) { SDL_BlitScaled(full, NULL, scaled, NULL); SDL_FreeSurface(full); full = scaled; }

    SDL_Texture *tex = SDL_CreateTextureFromSurface(app->renderer, full);
    SDL_FreeSurface(full); if (!tex) return NULL;
...
char* open_file_dialog(void) {
    static char fp[MAX_PATH_LENGTH]; fp[0] = '\0';
#ifdef _WIN32
    OPENFILENAME ofn = {0};
    ofn.lStructSize = sizeof(ofn); ofn.lpstrFile = fp; ofn.nMaxFile = sizeof(fp);
    ofn.lpstrFilter = "Image Files\0*.png;*.jpg;*.jpeg;*.bmp;*.gif;*.tga;*.webp\0All Files\0*.*\0";
    ofn.nFilterIndex = 1; ofn.lpstrTitle = "Open Image";
    ...
    if (fgets(fp, sizeof(fp), f)) {
        size_t l = strlen(fp); if (l && fp[l-1] == '\n') fp[l-1] = '\0';
    }
    pclose(f); return fp[0] ? fp : NULL;
#else
    FILE *f = popen("zenity --file-selection --title='Open Image' "
                    "--file-filter='Images | *.png *.jpg *.jpeg *.bmp *.gif *.tga *.webp' "
                    "2>/dev/null", "r");
    if (!f)
        f = popen("kdialog --getopenfilename . "
...
int load_image(App *app, const char *path) {
    ...
    if (app->image_texture)
        { SDL_DestroyTexture(app->image_texture); app->image_texture = NULL; }
...
void navigate_to(App *app, int index) {
    ...
    if (load_image(app, path)) {
        ...
        app->fit_to_window = 1; app->zoom = 1.0f;
        app->pan_x = 0; app->pan_y = 0; app->rotation = 0;
...
void copy_to_clipboard(App *app) {
...
#ifdef _WIN32
    SDL_Surface *surf = IMG_Load(app->current_path); if (!surf) return;
    SDL_Surface *bgr  = SDL_ConvertSurfaceFormat(surf, SDL_PIXELFORMAT_BGR24, 0);
    SDL_FreeSurface(surf); if (!bgr) return;
    ...
    BYTE *src = (BYTE*)bgr->pixels, *dst = mem + sizeof(bih);
    for (int y = h - 1; y >= 0; y--)
        memcpy(dst + (h-1-y)*dst_pitch, src + y*bgr->pitch, w*3);
    GlobalUnlock(hMem); SDL_FreeSurface(bgr);
...
void delete_current_image(App *app) {
    ...
    const char *fname = strrchr(path, PATH_SEP); fname = fname ? fname + 1 : path;
    ...
    SDL_MessageBoxData mbd = {SDL_MESSAGEBOX_WARNING, app->window,
        "Delete Image", msg, 2, btns, NULL};
    int btn = 0; SDL_ShowMessageBox(&mbd, &btn);
    if (btn != 1) return;
...
void render_image(App *app) {
    ...
    int eff_w = (app->rotation==90||app->rotation==270) ? app->image_height : app->image_width;
    int eff_h = (app->rotation==90||app->rotation==270) ? app->image_width  : app->image_height;
    int area_h = app->show_thumbnails ? app->window_height - THUMB_STRIP_H : app->window_height;

    SDL_Rect dest;
    if (app->fit_to_window) {
        float ar = (float)eff_w/eff_h, war = (float)app->window_width/area_h;
        if (ar > war) {
            dest.w = app->window_width; dest.h = (int)(app->window_width / ar);
            dest.x = 0;                dest.y = (area_h - dest.h) / 2;
        } else {
            dest.h = area_h; dest.w = (int)(area_h * ar);
            dest.x = (app->window_width - dest.w) / 2; dest.y = 0;
        }
    } else {
        dest.w = (int)(eff_w * app->zoom); dest.h = (int)(eff_h * app->zoom);
...
void render_info_panel(App *app) {
    ...
    // Dimensions
    char dim_buf[32];
    snprintf(dim_buf, sizeof(dim_buf), "%d × %d px", app->image_width, app->image_height);
    ...
    char ar_buf[32]; snprintf(ar_buf, sizeof(ar_buf), "%.2f : 1", ar);
    ...
    // Zoom
    char zoom_buf[32]; snprintf(zoom_buf, sizeof(zoom_buf), "%.0f%%", app->zoom * 100.f);
    draw_text(app, app->font_bold,    "Zoom",        tx,       ty, grey);
    draw_text(app, app->font_regular, zoom_buf,      tx + 70,  ty, white);
    ty += INFO_LINE_H;

    // Rotation
    char rot_buf[16]; snprintf(rot_buf, sizeof(rot_buf), "%d°", app->rotation);
...
void render_thumbnail_strip(App *app) {
    ...
    SDL_Rect line = {0, strip_y, app->window_width, 2};
    SDL_RenderFillRect(app->renderer, &line);

    int visible = app->window_width / THUMB_SLOT_W; if (visible < 1) visible = 1;
    int start = app->file_list.current - visible / 2;
    if (start < 0) start = 0;
    if (start + visible > app->file_list.count) start = app->file_list.count - visible;
    if (start < 0) start = 0;
    ...
    for (int i = start; i < start + visible && i < app->file_list.count; i++) {
        ...
        if (tex) {
            ...
            float ar = (float)tw / th;
            SDL_Rect dst;
            if (ar > (float)THUMB_W/THUMB_H) {
                dst.w = THUMB_W; dst.h = (int)(THUMB_W / ar);
                dst.x = x;       dst.y = strip_y + THUMB_PAD + (THUMB_H - dst.h)/2;
            } else {
                dst.h = THUMB_H; dst.w = (int)(THUMB_H * ar);
                dst.x = x + (THUMB_W - dst.w)/2; dst.y = strip_y + THUMB_PAD;
            }
            SDL_RenderCopy(app->renderer, tex, NULL, &dst);
        }
...
// Keyboard hint bar along the bottom of the image area
void render_hint_bar(App *app) {
    if (!app || !app->font_regular) return;
    static const char *hints =
        "O: Open   ←/→: Navigate   R: Rotate   I: Info   T: Thumbnails   "
        "Ctrl+C: Copy   Del: Delete   Drag & Drop supported";
...
void handle_events(App *app) {
    ...
    while (SDL_PollEvent(&ev)) {
        ...
        case SDL_QUIT: app->running = 0; break;
        ...
        case SDL_DROPFILE:
            if (ev.drop.file) { open_image_path(app, ev.drop.file); SDL_free(ev.drop.file); }
            break;
        ...
        case SDL_KEYDOWN: {
            ...
            switch (ev.key.keysym.sym) {
                ...
                case SDLK_PLUS:
                case SDLK_EQUALS: app->zoom *= 1.2f; app->fit_to_window = 0; break;
                case SDLK_MINUS:  app->zoom /= 1.2f; app->fit_to_window = 0; break;
                case SDLK_f:
                    app->fit_to_window=1; app->zoom=1.f; app->pan_x=0; app->pan_y=0; break;
                case SDLK_1:
                    app->fit_to_window=0; app->zoom=1.f; app->pan_x=0; app->pan_y=0; break;
                case SDLK_i: app->show_info       = !app->show_info;       break;
                case SDLK_t: app->show_thumbnails = !app->show_thumbnails; break;
                case SDLK_o: open_image(app); break;
                case SDLK_LEFT:   navigate_image(app, -1); break;
                case SDLK_RIGHT:  navigate_image(app,  1); break;
                case SDLK_r:
                    app->rotation = (app->rotation + (shift ? 270 : 90)) % 360; break;
                case SDLK_c:
                    if (ctrl) copy_to_clipboard(app); break;
                case SDLK_DELETE: delete_current_image(app); break;
            }
            break;
        }
        ...
        case SDL_MOUSEBUTTONDOWN:
            if (ev.button.button == SDL_BUTTON_LEFT) {
                if (app->show_thumbnails &&
                    ...
                    app->is_panning   = 1;
                    app->drag_start_x = ev.button.x; app->drag_start_y = ev.button.y;
                    app->pan_start_x  = app->pan_x;  app->pan_start_y  = app->pan_y;
                    app->fit_to_window = 0;
        ...
        case SDL_MOUSEBUTTONUP:
            if (ev.button.button == SDL_BUTTON_LEFT) app->is_panning = 0;
            break;
...
int initialize_sdl(App *app) {
    ...
    if (!app->font_regular)
        SDL_Log("Warning: Could not load font (%s). Text will not be rendered.", TTF_GetError());

    app->running = 1; app->zoom = 1.0f; app->fit_to_window = 1;
    app->show_info = 0; app->show_thumbnails = 1; app->rotation = 0;
    return 1;
}
